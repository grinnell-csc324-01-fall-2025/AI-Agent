...
router.get('/', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    console.log(`[Messages API] [${new Date().toISOString()}] Fetching messages for userId: ${userId}`);
    ...
    console.log(`[Messages API] [${new Date().toISOString()}] Successfully fetched ${messages.length} messages for userId: ${userId} (${duration}ms)`);
    ...
    console.error(`[Messages API] [${new Date().toISOString()}] Error fetching messages:`, errorDetails);
