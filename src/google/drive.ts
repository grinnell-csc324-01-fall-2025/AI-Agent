...
export async function listRecentFiles(userId: string, retryCount = 0): Promise<any[]> {
  if (!userId || typeof userId !== 'string' || userId.length !== 24) {
    throw new Error(`Invalid userId provided to listRecentFiles: ${userId}. Expected 24-character MongoDB ObjectId.`);
  }
  
  const startTime = Date.now();
  console.log(`[Drive API] [${new Date().toISOString()}] Fetching recent files for userId: ${userId}, retryCount: ${retryCount}`);
  ...
  try {
    ...
    console.error(`[Drive API] [${new Date().toISOString()}] Failed to get OAuth client for user ${userId}:`, errorDetails);
    throw authError;
  }
  
  // Verify credentials are set
  if (!auth.credentials?.access_token) {
    console.error(`[Drive API] [${new Date().toISOString()}] No access token available after getting OAuth client for user ${userId}`);
    throw new Error('No access token available. Please re-authenticate.');
  }
  
  console.log(`[Drive API] [${new Date().toISOString()}] Making request with access token for user ${userId}`);
  ...
  for (let attempt = 0; attempt <= retryCount; attempt++) {
    try {
      if (attempt > 0) {
        ...
        console.log(`[Drive API] Retry attempt ${attempt + 1}/${retryCount + 1}, waiting ${waitTime / 1000}s...`);
      ...
      console.log(`[Drive API] [${new Date().toISOString()}] Successfully fetched ${validFiles.length} files for user ${userId} (${duration}ms)`);
      ...
      console.error(`[Drive API] [${new Date().toISOString()}] Error fetching files for user ${userId} (attempt ${attempt + 1}):`, errorDetails);
  ...
  if (!error) {
    throw new Error('Unknown error occurred while fetching files from Google Drive');
  }
  ...
  console.error(`[Drive API] [${new Date().toISOString()}] Failed to fetch files after ${retryCount + 1} attempts for user ${userId}:`, errorDetails);
  ...
  if (error?.code === 403) {
    if (error?.message?.includes('unregistered callers') || error?.response?.data?.error?.message?.includes('API not enabled')) {
      ...
      throw new Error('Insufficient permissions to access Google Drive. Please ensure the app has the required Drive scopes and re-authenticate.');
    } else {
      throw new Error(`Access denied to Google Drive API (403). ${error?.message || error?.response?.data?.error?.message || 'Please check your permissions and API settings.'}`);
    }
  ...
  const errorMessage = error?.message || error?.response?.data?.error?.message || 'Unknown error occurred while fetching files from Google Drive';
